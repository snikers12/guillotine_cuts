...
def get_details():
    ...
    details = sorted(details, key=lambda item: (-item['a'], -item['a'] * item['b']))
...
def recursive():
    ...
    if not isinstance(sheet, bool):
        ...
        for detail in details:
            ...
            if (get_orient(sheet, detail, True) and cut_horizontal(sheet, detail, True)) or \
                    (get_orient(sheet, detail, True) and cut_vertical(sheet, detail, True)) or \
                    (get_orient(sheet, detail, False) and cut_horizontal(sheet, detail, True)) or \
                    (get_orient(sheet, detail, False) and cut_vertical(sheet, detail, True)):
                result = True
                break
        if not result:
